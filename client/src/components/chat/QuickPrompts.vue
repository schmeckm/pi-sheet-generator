<template>
  <div class="joule-quick-prompts">
    ...
    <p class="joule-quick-prompts__subhint">{{ t('prompts.movementHint') }}</p>
    <div class="joule-quick-prompts__grid">
      <button
        v-for="item in movementPrompts"
        :key="item.key"
        type="button"
    ...
    <p class="joule-quick-prompts__heading joule-quick-prompts__heading--spaced">
      {{ t('prompts.processHint') }}
    </p>
    <div class="joule-quick-prompts__grid">
      <button
        v-for="item in processPrompts"
        :key="item.key"
        type="button"
        class="joule-suggestion-card"
        @click="$emit('select', item.text)"
...
const movementMeta = [
  { key: 'packagingEwm', icon: 'warehouse' },
  { key: 'packagingMm', icon: 'movement' },
  { key: 'packagingConfirmations', icon: 'reply' },
];
...
const movementPrompts = computed(() => mapPrompts(movementMeta));
const processPrompts = computed(() => mapPrompts(processMeta));
const equipmentPrompts = computed(() => mapPrompts(equipmentMeta));
</script>
